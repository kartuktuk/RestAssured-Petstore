...
import Petstore.api.enpoints.UserEndpointsFromProperties;
...
public class UserTestsDrivingFromProperties {
    Faker faker;
    User userPayload;
    ...
    public void testUpdateUserByName() {
        ...
        userPayload.setLastName(faker.name().lastName());
        userPayload.setEmail(faker.internet().safeEmailAddress());
        Response response = UserEndpointsFromProperties.updateUser(userPayload, this.userPayload.getUsername());
        response.then().log().body();

        Assert.assertEquals(response.getStatusCode(), 200);
        logger.info("************  USER UPDATED *******************");

        Response responseAfterUpdate = UserEndpointsFromProperties.getUser(this.userPayload.getUsername());
